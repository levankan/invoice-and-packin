...
def export_database_excel(request):
    ...
    headers = [
        ...
        "Invoice No",
        "Packing List No",
        "Per-Pallet Invoice No",
        "Per-Pallet Packing List No",
        "Export No",
        "Project No",
        "Created At",
        "Created By",
    ...
    for exp in Export.objects.all().prefetch_related("items").order_by("id"):
        for item in exp.items.all():
            ws.append([
                ...
                f"{exp.invoice_number or ''}/{item.pallet_number or ''}" if item.pallet_number else exp.invoice_number or "",
                f"{exp.packing_list_number or ''}/{item.pallet_number or ''}" if item.pallet_number else exp.packing_list_number or "",
