...
def edit_import(request, pk):
    ...
    def _build_packages_context(import_obj):
        ...
        for p in packages_qs:
            packages_list_local.append(
                {
                    "type": p.package_type or "",
                    "length": p.length_cm,  # stored as cm
                    "width": p.width_cm,
                    "height": p.height_cm,
                    "gross_weight": p.gross_weight_kg,  # stored as kg
                    "unit_system": "metric",  # DB uses metric
                }
            )
        return packages_list_local, json.dumps(packages_list_local, default=str)

    def _build_lines_context(import_obj):
        lines_qs = import_obj.lines.all().order_by("pk")
        lines_list_local = []
        for l in lines_qs:
            lines_list_local.append(
                {
                    ...
                    "expected_receipt_date": l.expected_receipt_date.isoformat()
                    if l.expected_receipt_date
                    else "",
                    "delivery_date": l.delivery_date.isoformat() if l.delivery_date else "",
                }
            )
        return lines_list_local, json.dumps(lines_list_local, default=str)
    ...
    if request.method == "POST":
        ...
        if action == "upload_lines":
            ...
            try:
                ...
                messages.success(
                    request,
                    f"{len(new_lines)} new line(s) uploaded and replaced successfully.",
                )
        ...
        if form.is_valid():
            ...
            with transaction.atomic():
                ...
                imp.total_volumetric_weight_kg = (
                    total_vol.quantize(Decimal("0.001")) if total_vol != 0 else None
                )
